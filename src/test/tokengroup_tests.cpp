...
static bool tryBlock(const std::vector<CMutableTransaction> &txns,
                             const CScript &scriptPubKey, CBlock& result)
{
    ...
    bool ret;
    ret = ProcessNewBlock(chainparams, NULL, &block, true, NULL, false);
    result = block;
    delete pblocktemplate;
    return ret;
}

static bool tryMempool(const CTransaction& tx)
{
    LOCK(cs_main);
    bool inputsMissing = false;
    return AcceptToMemoryPool(mempool, tx, false, &inputsMissing, true, false);
}
...
BOOST_FIXTURE_TEST_CASE(tokengroup_blockchain, TestChain100Setup)
{
    ...
    bool ret = tryBlock(txns,p2pkh(grp1.addr),blk1);
    BOOST_CHECK(ret);
    ...
    ret = tryBlock(txns,p2pkh(a2.addr), badblk);
    BOOST_CHECK(!ret);
    }
    ...
    txns[0] = tx1x1(COutPoint(coinbaseTxns[0].GetHash(),0), p2pkh(grp0.addr), coinbaseTxns[0].vout[0].nValue,
                    coinbaseKey, coinbaseTxns[0].vout[0].scriptPubKey, false);
    ret = tryBlock(txns,p2pkh(a2.addr), tipblk);
    BOOST_CHECK(ret);
