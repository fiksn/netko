...
bool CheckTokenGroups(const CTransaction &tx, const MapPrevTx& inputs)
{
    ...
    {
        /*TODO(fiction) check whether this is correct */
        const CTxOut& out = tx.GetOutputFor(inp, inputs);
        ...
        else
        {
            ...
            if (item == gBalance.end())  // no output group matches the input, so this must be a burn
            {
                ...
                {
                    return error("Group: Token burn is not signed by group id");
                }
                else
                {
                    gBalance[BitcoinGroup].input += amount;
    ...
    for (auto& txo: gBalance)
    {
        ...
        if (bal.input < bal.output)  // coins must be minted or melted
        {
            ...
            {
                return error("Group output exceeds input, including all mintable");
            }
        }
    ...
    {
        return error("Group transaction imbalance");
    }
